...
* One potential issue with opt-in disclosure protection is that the noise added to the opt-in data will diminish the gains of including unaltered observations. For example, as opt-in decreases the number of observations in the training data declines and could lead to terrible models for the synthetic data.
* We do not witness this phenomena in demonstration 2. 
* Our results support the idea that opt-in approaches to synthetic data can improve data quality with minimal changes to disclosure risks. 
* All utility metrics improve with the introduction of an opt-in and improve as the rate of opt-in declines.
* Most of the utility metrics are surprisingly resilient to differenital opt-in.

MULTIVARIATE UTILITY

* We start with multivariate utility. The discrimnator AUC is very high when opt-in = 1 but quickly declines with lower opt-in rates. 
* This suggests that a simple predictive model has difficulty distinguishing between the synthetic data and the GSDS.
* The correlation difference is modest for all opt-in rates and improves with low levels of opt-in. 

UNIVARIATE UTILITY

* We next zoom in on specific variables and subsets of variables. 

* The categorical utility of the synthetic data is very high for all scenarios but improves marginally with low opt-in rates. 

* The synthetic data largely recreat proportions from the GSDS. 
* The results follow a same pattern when disaggregated by race group. Here, the small groups are very imprecise. Note that low levels for the white multiplier dramatically improve the results for small race groups. 

* TODO: calculate kmarginals with the holdout data

* The synthesis is mediocre overall for the numeric variables. These variables would benefit from some tuning of the general synthesis approach. 
* Still, we see dramatic improvements for low opt-in rates. 

* The distribution of wage and salary income improves dramatically under the opt-in scenario. Some percentiles ar
...
* SUMMARY SENTENCE
* The AUC for the membership inference attack is close to 0.5. Given the synthetic data, an attacker would struggle to tell if a confidential record is from the GSDS or holdout data using distance-based matching. 
VISUALIZATION
* Finally, the RMSE for the attribute inference test is modest. In fact, it is smaller for the model trained on the holdout data than for the model trained on the synthetic data. Using a simple predictive model, this suggests that the synthetic data are not copying chance features of the GSDS. 
* SUMMARY SENTENCE
* These disclosure metrics enable us to infer the disclosive properties of the synthetic but they don't not provide a bound or guarantee on the disclosure risks of the synthetic data. 
* We believe the results are promising. Allowing individuals to forego disclosure protections increases data utility without increasing disclosure risks for the individuals who still want disclosure protection. 
* This idea rests on a few assumptions. For example, we do not label observations that forego disclosure protections. Doing so could increase disclosure risks by making differencing attacks easier. 
* Data stewards need to be careful to not implicitly label the opt-in decision. For example, the GSDS could have precision to the closest integer but synthetic records go to three decimal places. 
...
* Next work
    * Ample followup work is needed to explore implementation of opt-in disclosure protection. We do not address questionnaire design and giving respondents clear information sot hey can provide informed consent. These implementation decisions inform the ethics of opt-in disclosure protection and would unambiguously affect the opt-in rate. 
    * SED
