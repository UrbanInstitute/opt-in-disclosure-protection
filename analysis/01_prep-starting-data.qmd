...
starting001 <- prep001$starting %>%
  add_opt_out_prob() %>%
  add_opt_out_decision(opt_out_prob, .5)
  
save(starting001, file = paste(here("data", "starting"), "starting-data-001.rda", sep = "/"))
save(lookup001, file = paste(here("data", "lookup"), "lookup-data-001.rda", sep = "/"))
```
