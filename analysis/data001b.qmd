...
results <- bind_rows(
  ...
  `global` = pmap_dfr(
    ...
    .f = function(run, epsilon) data |>
      ...
      uncount(weights = nrow(unique(select(specs, c(opt_in, white_multiplier))))) %>% 
      mutate(opt_in = rep(unique(specs$opt_in), length.out = n()),
             white_multiplier = rep(unique(specs$white_multiplier), length.out = n()))
    ),
  .id = "method"
)

saveRDS(results, file = here("data", "results", "results-001b.rds"))
```
